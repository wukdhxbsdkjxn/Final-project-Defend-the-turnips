...
void ForTowerManager::showTowerOptions(const Vec2& position)
{
    if (!options.build) {
...
void ForTowerManager::showUP_BOTTLEOptions(const Vec2& position, const int& time)
{
    cocos2d::Vec2 newPosition = cocos2d::Vec2(position.x, position.y + 50);
    /////////////////////////////////
    if (options.up) {
...
void ForTowerManager::showUP_SHITOptions(const Vec2& position, const int& time)//flyScene中传递进来time
{
    cocos2d::Vec2 newPosition = cocos2d::Vec2(position.x, position.y + 50);
    /////////////////////////////////
    if (options.up) {
...
void ForTowerManager::showUP_PINOptions(const Vec2& position, const int& time)//flyScene中传递进来time
{
    cocos2d::Vec2 newPosition = cocos2d::Vec2(position.x, position.y + 50);
    /////////////////////////////////
    if (options.up) {
    ...
    createUP_PINOptions(position, time);//显示升级选项
...
void ForTowerManager::createbottleTower(const Vec2& position, const int& time)
{
...
void ForTowerManager::createshitTower(const Vec2& position, const int& time)
{
    CocosDenshion::SimpleAudioEngine::getInstance()->preloadBackgroundMusic("3.mp3");
    // 播放背景音乐
    CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("3.mp3", false);
...
void ForTowerManager::createpinTower(const Vec2& position, const int& time)
{
    CocosDenshion::SimpleAudioEngine::getInstance()->preloadBackgroundMusic("3.mp3");
    // 播放背景音乐
    CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("3.mp3", false);
    ...
    if (time == 0 && selectedTowerType != Tower::Type::None)
    {
        if (flyScene->wealth >= 160) {
            selectedTowerType = Tower::Type::BasicPin;
            auto tower = Tower::create(Tower::Type::BasicPin);
            tower->setPosition(position);
            tower->tower_locx = position.x;
            tower->tower_locy = position.y;
            tower->distance = 150;
            ...
            getParent()->addChild(tower, 1);  // 添加到FlyScene中
            flyScene->wealth -= 160;
            flyScene->tower_change(flyScene->loc_return(position.x, position.y), 1, 3);
            flyScene->towerList.push_back(tower);
        }
        else
            noTower();
    ...
    if (time == 1 && selectedTowerType != Tower::Type::None)
    {
        if (flyScene->wealth >= 260) {
            // 删除点击位置的炮塔
            removeTowerAtPosition(position);
            selectedTowerType = Tower::Type::BetterPin;
            auto tower = Tower::create(Tower::Type::BetterPin);
            tower->setPosition(position);
            tower->tower_locx = position.x;
            tower->tower_locy = position.y;
            tower->distance = 160;
            ...
            tower->type = 3;
            getParent()->addChild(tower, 1);// 添加到FlyScene中
            flyScene->tower_change(flyScene->loc_return(position.x, position.y), 2, 3);
            flyScene->wealth -= 260;
            flyScene->towerList.push_back(tower);
        }
        else
    ...
    if (time == 2 && selectedTowerType != Tower::Type::None)
    {
        if (flyScene->wealth >= 260) {
            // 删除点击位置的炮塔
            removeTowerAtPosition(position);
            selectedTowerType = Tower::Type::BestPin;
            auto tower = Tower::create(Tower::Type::BestPin);
            tower->setPosition(position);
            tower->tower_locx = position.x;
            tower->tower_locy = position.y;
            tower->distance = 220;
            ...
            getParent()->addChild(tower, 1);  // 添加到FlyScene中
            flyScene->wealth -= 260;
            flyScene->tower_change(flyScene->loc_return(position.x, position.y), 3, 3);
            flyScene->towerList.push_back(tower);
        }
        else
            noTower();
...
bool ForTowerManager::onTouchBegan(const Touch* touch, const Event* event)//这里有问题，出现升级选项时
{
    // 获取点击位置
...
void ForTowerManager::createOptions(const Vec2& position)
{
    if (options.build) {
    ...
    option2->setLocalZOrder(1);
    auto option3 = MenuItemImage::create(
        "pin0build.png",
        "buildA.png",
        [=](Ref* sender) {
            selectedTowerType = Tower::Type::BasicPin;
            createpinTower(position, 0);  // 选择炮塔后直接创建
            removeTowerOptions();
        });
    option3->setLocalZOrder(1);

    // 设置选项的位置
    option2->setPosition(Vec2(-70, 0));
    option1->setPosition(Vec2(70, 0));
    option3->setPosition(Vec2(0, 0));

    // 将选项添加到菜单中
    options.build->addChild(option1, 1);
    options.build->addChild(option2, 1);
    options.build->addChild(option3, 1);

    // 注册触摸事件
    auto touchListener = EventListenerTouchOneByOne::create();
    touchListener->setSwallowTouches(true);
    touchListener->onTouchBegan = CC_CALLBACK_2(ForTowerManager::onTouchBegan, this);
    _eventDispatcher->addEventListenerWithSceneGraphPriority(touchListener, this);
}

void ForTowerManager::createUP_BOTTLEOptions(const Vec2& position, const int& time)
{
    if (options.up) {/////
...
void ForTowerManager::createUP_SHITOptions(const Vec2& position, const int& time)
{
    if (options.up) {
        options.up->removeFromParentAndCleanup(true);
    }
...
void ForTowerManager::createUP_PINOptions(const Vec2& position, const int& time)
{
    if (options.up) {/////
        options.up->removeFromParentAndCleanup(true);
    }
    ...
    auto option1 = MenuItemImage::create(
        ...
        [=](Ref* sender) {
            selectedTowerType = Tower::Type::BetterPin;
            createpinTower(position, time);  // 选择炮塔后直接创建
            removeTowerOptions();
        });
    option1->setLocalZOrder(1);
    auto option2 = MenuItemImage::create(
...
void ForTowerManager::DELETETOptions(const Vec2& position) {
...
void ForTowerManager::removeTowerOptions()
{
...
void ForTowerManager::removeTowerAtPosition(const Vec2& position)
{
    auto flyScene = dynamic_cast<ForestScene*>(this->getScene());
    if (flyScene)
    {
